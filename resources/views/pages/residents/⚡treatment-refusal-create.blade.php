...
use App\Concerns\TreatmentRefusalValidationRules;
...
#[Title('Document Treatment Refusal')]
class extends Component {
    use TreatmentRefusalValidationRules;

    #[Locked]
    public int $residentId;

    public string $refusal_date = '';
    ...
    public function mount(Resident $resident): void
    {
        ...
        $this->refusal_date = now()->format('Y-m-d');
    }
    ...
    public function useAndSaveSignature(string $dataUrl, string $penColor): void
    {
        ...
        $sig = $user->signatures()->create([
            'name'           => 'Treatment Refusal — ' . now()->format('M d, Y'),
            'pen_color'      => $penColor,
    ...
    {
        if (! $this->isSigned) {
            $this->addError('signature', 'A staff signature is required to save this record.');
            return;
        }
        $v = $this->validate($this->treatmentRefusalRules());
        TreatmentRefusal::create([
            'resident_id'        => $this->residentId,
            'refusal_date'       => $v['refusal_date'],
            'illness_description'=> $v['illness_description'] ?? null,
            'signers'            => $v['signers'] ?? [],
            'signature_id'      => $this->signature_id,
            'raw_signature_data'=> ($this->signature_id === null && $this->rawSignatureData !== '') ? $this->rawSignatureData : null,
            'recorded_by'       => auth()->id(),
        ]);
        session()->flash('status', 'Treatment refusal documented successfully.');
        $this->redirect(route('residents.treatment-refusals.index', $this->residentId), navigate: true);
    }
}; ?>
...
<flux:main>
    <div class="max-w-4xl space-y-1">
        ...
        <div class="mb-6 flex items-center gap-3">
            <flux:button variant="ghost" :href="route('residents.treatment-refusals.index', $this->residentId)" wire:navigate icon="arrow-left" />
            <div>
                <flux:heading size="xl">{{ __('Document Treatment Refusal') }}</flux:heading>
        ...
        <form wire:submit="save" class="space-y-4">
            ...
            <flux:card class="space-y-4">
                <div class="flex items-center gap-2">
                    <flux:icon name="no-symbol" class="size-5 text-accent" />
                    <flux:heading size="sm">{{ __('Refusal Details') }}</flux:heading>
                </div>
                <flux:separator />
                <flux:input type="date" wire:model="refusal_date" :label="__('Refusal Date')" required />
                @error('refusal_date') <flux:text class="text-sm text-red-600 dark:text-red-400">{{ $message }}</flux:text> @enderror
                <flux:textarea wire:model="illness_description" :label="__('Illness / Injury Description')" rows="4" placeholder="{{ __('Describe the illness or injury for which treatment is being refused...') }}" />
            </flux:card>
            ...
            <flux:card class="space-y-3">
                <div class="flex items-center gap-2">
                    ...
                    <flux:heading size="sm">{{ __('Resident Statement') }}</flux:heading>
                </div>
                <flux:separator />
                <div class="rounded-lg border border-amber-200 bg-amber-50 p-4 dark:border-amber-700/40 dark:bg-amber-900/20">
                    <flux:text class="text-sm leading-relaxed text-zinc-700 dark:text-zinc-300">
                        {{ __('I have been offered the opportunity to have medical care at the doctors office, hospital, urgent care for the above illness/injury. I feel as though I do not require medical care at this time. However, should I feel the need to have medical care, I will immediately report this to a staff.') }}
                    </flux:text>
                </div>
            </flux:card>
            ...
            <flux:card class="space-y-4">
                ...
                @if ($this->mySignatures->isEmpty())
                    <flux:text class="text-sm text-zinc-500 dark:text-zinc-400">{{ __('Draw your signature below.') }}</flux:text>
                    <div
                        x-data="{
                            ...
                            <label class="cursor-pointer" wire:key="sig-{{ $sig->id }}">
                                ...
                                <div class="rounded-xl border-2 p-3 transition" :class="$wire.signature_id === {{ $sig->id }} ? 'border-accent bg-accent/5 ring-2 ring-accent/20' : 'border-zinc-200 hover:border-zinc-300 dark:border-zinc-700 dark:hover:border-zinc-600'">
                                    ...
                                    <div class="mt-2 flex items-center gap-1.5">
                                        <span class="size-3 shrink-0 rounded-full border border-zinc-200" style="background-color: {{ $sig->pen_color }}"></span>
            ...
            <div class="flex justify-end gap-3 pb-4">
                <flux:button variant="ghost" :href="route('residents.treatment-refusals.index', $this->residentId)" wire:navigate>{{ __('Cancel') }}</flux:button>
