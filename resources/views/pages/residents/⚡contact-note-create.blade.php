...
use App\Concerns\ContactNoteValidationRules;
...
class extends Component {
    ...
    public array $person_contacted = [];
    public string $contact_name = '';
    public array $mode_of_contact = [];
    public string $mode_other = '';
    public string $contact_summary = '';
    public string $emergency_issue = '';
    ...
    public function mount(Resident $resident): void
    {
        ...
        $this->contact_date = now()->format('Y-m-d');
    }
    ...
    public function save(): void
    {
        ...
        $v = $this->validate($this->contactNoteRules());
        ContactNote::create([
            'resident_id'      => $this->residentId,
            'diagnosis'        => $v['diagnosis'] ?? null,
            'contact_date'     => $v['contact_date'],
            'contact_time'     => $v['contact_time'] ?? null,
            'person_contacted' => $v['person_contacted'] ?? [],
            'contact_name'     => $v['contact_name'] ?? null,
            'mode_of_contact'  => $v['mode_of_contact'] ?? [],
            'mode_other'       => $v['mode_other'] ?? null,
            'contact_summary'  => $v['contact_summary'] ?? null,
            'emergency_issue'  => $this->emergency_issue !== '' ? (bool) $this->emergency_issue : null,
            'signers'          => $v['signers'] ?? [],
...
<flux:main>
    <div class="max-w-4xl space-y-1">
        ...
        <form wire:submit="save" class="space-y-4">
            ...
            <flux:card class="space-y-4">
                <div class="flex items-center gap-2">
                    <flux:icon name="phone" class="size-5 text-accent" />
                    <flux:heading size="sm">{{ __('Contact Details') }}</flux:heading>
                </div>
                <flux:separator />
                <div class="grid gap-4 sm:grid-cols-2">
                    <flux:input type="date" wire:model="contact_date" :label="__('Contact Date')" required />
                    <flux:input type="time" wire:model="contact_time" :label="__('Contact Time')" />
                </div>
                @error('contact_date') <flux:text class="text-sm text-red-600 dark:text-red-400">{{ $message }}</flux:text> @enderror

                <div>
                    <flux:label>{{ __('Person Contacted') }}</flux:label>
                    <div class="mt-2 flex flex-wrap gap-2">
                        @foreach (['Guardian', 'Case Manager', 'Pharmacy', 'Family member', 'Doctors office', 'Other'] as $option)
                            <label class="cursor-pointer select-none">
                                <input type="checkbox" wire:model="person_contacted" value="{{ $option }}" class="sr-only" />
                                <span class="inline-block rounded-full border px-3 py-1.5 text-sm font-medium transition"
                                    :class="$wire.person_contacted.includes('{{ $option }}') ? 'border-accent bg-accent/10 text-accent ring-1 ring-accent/30' : 'border-zinc-200 text-zinc-600 hover:border-zinc-300 hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-400'">
                                    {{ $option }}
                                </span>
                            </label>
                        @endforeach
                ...
                <flux:input wire:model="contact_name" :label="__('Contact Name')" placeholder="{{ __('Full name of person contacted...') }}" />
            </flux:card>
            ...
            <flux:card class="space-y-4">
                <div class="flex items-center gap-2">
                    ...
                    <flux:heading size="sm">{{ __('Mode of Contact') }}</flux:heading>
                </div>
                <flux:separator />
                <div class="flex flex-wrap gap-2">
                    @foreach (['Email', 'Text message', 'Phone call', 'In person', 'Other'] as $option)
                        <label class="cursor-pointer select-none">
                            <input type="checkbox" wire:model="mode_of_contact" value="{{ $option }}" class="sr-only" />
                            <span class="inline-block rounded-full border px-3 py-1.5 text-sm font-medium transition"
                                :class="$wire.mode_of_contact.includes('{{ $option }}') ? 'border-accent bg-accent/10 text-accent ring-1 ring-accent/30' : 'border-zinc-200 text-zinc-600 hover:border-zinc-300 hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-400'">
                                {{ $option }}
                            </span>
                        </label>
                    @endforeach
                </div>
                @if (in_array('Other', $mode_of_contact))
                    <flux:input wire:model="mode_other" :label="__('Other Mode (specify)')" placeholder="{{ __('Describe the mode of contact...') }}" />
                @endif
            </flux:card>
            ...
            <flux:card class="space-y-4">
                ...
                <flux:separator />
                <flux:textarea wire:model="contact_summary" :label="__('Contact Summary')" rows="5" />

                <div>
                    <flux:label>{{ __('Emergency Issue?') }}</flux:label>
                    <div class="mt-2 flex gap-2">
                        @foreach (['1' => 'Yes', '0' => 'No'] as $val => $label)
                            <label class="cursor-pointer select-none">
                                <input type="radio" wire:model.live="emergency_issue" value="{{ $val }}" class="sr-only" />
                                <span @class([
                                    'inline-block rounded-full border px-4 py-1.5 text-sm font-medium transition',
                                    'border-accent bg-accent/10 text-accent ring-1 ring-accent/30' => $emergency_issue === $val,
                                    'border-zinc-200 text-zinc-600 hover:border-zinc-300 hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-400' => $emergency_issue !== $val,
                                ])>{{ $label }}</span>
                            </label>
                        @endforeach
                    </div>
            ...
            <div class="flex justify-end gap-3 pb-4">
                <flux:button variant="ghost" :href="route('residents.contact-notes.index', $this->residentId)" wire:navigate>{{ __('Cancel') }}</flux:button>
