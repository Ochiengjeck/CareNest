<?php

use App\Models\ArtMeeting;
use App\Models\Resident;
use Livewire\Attributes\Computed;
use Livewire\Attributes\Layout;
use Livewire\Attributes\Locked;
use Livewire\Attributes\Title;
use Livewire\Component;
use Livewire\WithPagination;

new
#[Layout('layouts.app.sidebar')]
#[Title('ART Meetings')]
class extends Component {
    use WithPagination;

    #[Locked]
    public int $residentId;

    public function mount(Resident $resident): void
    {
        abort_unless(auth()->user()->can('view-residents'), 403);
        $this->residentId = $resident->id;
    }

    #[Computed]
    public function resident(): Resident
    {
        return Resident::findOrFail($this->residentId);
    }

    #[Computed]
    public function records()
    {
        return ArtMeeting::where('resident_id', $this->residentId)
            ->with(['recorder'])
            ->latest('meeting_date')
            ->paginate(15);
    }

    #[Computed]
    public function totalRecords(): int
    {
        return ArtMeeting::where('resident_id', $this->residentId)->count();
    }

    #[Computed]
    public function recordsThisMonth(): int
    {
        return ArtMeeting::where('resident_id', $this->residentId)
            ->whereMonth('meeting_date', now()->month)
            ->whereYear('meeting_date', now()->year)
            ->count();
    }

    #[Computed]
    public function upcomingCount(): int
    {
        return ArtMeeting::where('resident_id', $this->residentId)
            ->whereNotNull('next_meeting_date')
            ->where('next_meeting_date', '>=', now()->toDateString())
            ->count();
    }
}; ?>

<flux:main>
    <div class="max-w-5xl space-y-6">

        <div class="flex flex-wrap items-start justify-between gap-4">
            <div class="flex items-center gap-3">
                <flux:button variant="ghost" :href="route('residents.show', $this->residentId)" wire:navigate icon="arrow-left" />
                <div>
                    <flux:heading size="xl">{{ __('ART Meetings') }}</flux:heading>
                    <flux:subheading>{{ __('Active Treatment Review — ') }}{{ $this->resident->full_name }}</flux:subheading>
                </div>
            </div>
            @can('manage-residents')
                <flux:button variant="primary" icon="plus" :href="route('residents.art-meetings.create', $this->residentId)" wire:navigate>
                    {{ __('New Meeting Note') }}
                </flux:button>
            @endcan
        </div>

        <div class="grid grid-cols-3 gap-4">
            <flux:card class="flex items-center gap-3 p-4">
                <div class="flex size-10 items-center justify-center rounded-full bg-blue-50 dark:bg-blue-900/20">
                    <flux:icon name="users" class="size-5 text-blue-600 dark:text-blue-400" />
                </div>
                <div>
                    <div class="text-xl font-bold text-zinc-800 dark:text-zinc-100">{{ $this->totalRecords }}</div>
                    <div class="text-xs text-zinc-500 dark:text-zinc-400">{{ __('Total Meetings') }}</div>
                </div>
            </flux:card>
            <flux:card class="flex items-center gap-3 p-4">
                <div class="flex size-10 items-center justify-center rounded-full bg-violet-50 dark:bg-violet-900/20">
                    <flux:icon name="calendar-days" class="size-5 text-violet-600 dark:text-violet-400" />
                </div>
                <div>
                    <div class="text-xl font-bold text-zinc-800 dark:text-zinc-100">{{ $this->recordsThisMonth }}</div>
                    <div class="text-xs text-zinc-500 dark:text-zinc-400">{{ __('This Month') }}</div>
                </div>
            </flux:card>
            <flux:card class="flex items-center gap-3 p-4">
                <div class="flex size-10 items-center justify-center rounded-full bg-green-50 dark:bg-green-900/20">
                    <flux:icon name="arrow-right-circle" class="size-5 text-green-600 dark:text-green-400" />
                </div>
                <div>
                    <div class="text-xl font-bold text-zinc-800 dark:text-zinc-100">{{ $this->upcomingCount }}</div>
                    <div class="text-xs text-zinc-500 dark:text-zinc-400">{{ __('Upcoming') }}</div>
                </div>
            </flux:card>
        </div>

        @if ($this->records->isEmpty())
            <flux:card class="flex flex-col items-center py-16 text-center">
                <div class="mb-4 flex size-16 items-center justify-center rounded-full bg-zinc-100 dark:bg-zinc-800">
                    <flux:icon name="users" class="size-8 text-zinc-400" />
                </div>
                <flux:heading size="sm">{{ __('No ART meetings yet') }}</flux:heading>
                <flux:text class="mt-1 max-w-sm text-sm text-zinc-400">
                    {{ __('Active Treatment Review meeting notes for this resident will appear here.') }}
                </flux:text>
                @can('manage-residents')
                    <flux:button class="mt-6" variant="primary" icon="plus" :href="route('residents.art-meetings.create', $this->residentId)" wire:navigate>
                        {{ __('Record First Meeting') }}
                    </flux:button>
                @endcan
            </flux:card>
        @else
            <flux:card class="overflow-hidden p-0">
                <flux:table>
                    <flux:table.columns>
                        <flux:table.column>{{ __('Meeting Date') }}</flux:table.column>
                        <flux:table.column>{{ __('Type') }}</flux:table.column>
                        <flux:table.column>{{ __('Attendees') }}</flux:table.column>
                        <flux:table.column>{{ __('Next Meeting') }}</flux:table.column>
                        <flux:table.column>{{ __('Recorded By') }}</flux:table.column>
                        <flux:table.column class="w-10"></flux:table.column>
                    </flux:table.columns>
                    <flux:table.rows>
                        @foreach ($this->records as $record)
                            <flux:table.row :key="$record->id">
                                <flux:table.cell>
                                    <div class="font-medium">{{ $record->meeting_date->format('M d, Y') }}</div>
                                    <div class="text-xs text-zinc-400">{{ $record->meeting_date->diffForHumans() }}</div>
                                </flux:table.cell>
                                <flux:table.cell>
                                    <flux:badge size="sm" :color="$record->meeting_type_color">{{ $record->meeting_type_label }}</flux:badge>
                                </flux:table.cell>
                                <flux:table.cell>
                                    @if (!empty($record->attendees))
                                        <span class="text-sm text-zinc-600 dark:text-zinc-400">{{ count($record->attendees) }} {{ __('attendee(s)') }}</span>
                                    @else
                                        <span class="text-zinc-400">—</span>
                                    @endif
                                </flux:table.cell>
                                <flux:table.cell>
                                    @if ($record->next_meeting_date)
                                        <span class="text-sm">{{ $record->next_meeting_date->format('M d, Y') }}</span>
                                    @else
                                        <span class="text-zinc-400">—</span>
                                    @endif
                                </flux:table.cell>
                                <flux:table.cell>{{ $record->recorder?->name ?? '—' }}</flux:table.cell>
                                <flux:table.cell>
                                    <flux:button variant="ghost" size="sm" icon="arrow-top-right-on-square"
                                        :href="route('art-meetings.show', $record->id)" wire:navigate />
                                </flux:table.cell>
                            </flux:table.row>
                        @endforeach
                    </flux:table.rows>
                </flux:table>
            </flux:card>
            <div>{{ $this->records->links() }}</div>
        @endif
    </div>

    @script
    <script>
        @if (session('status'))
            Flux.toast({ text: '{{ session('status') }}', variant: 'success' });
        @endif
    </script>
    @endscript
</flux:main>
