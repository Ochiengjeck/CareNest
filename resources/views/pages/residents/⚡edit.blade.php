...
class extends Component {
    ...
    public function nextStep(): void
    {
        $rules = match ($this->currentStep) {
            ...
            2 => [...$this->admissionRules(), ...$this->dischargeDateRules(), ...$this->medicalRules()],
    ...
    public function removePhoto(): void
    {
        if ($this->existing_photo) {
            Storage::delete($this->existing_photo);
    ...
    public function save(): void
    {
        ...
        if ($this->photo) {
            ...
            $data['photo_path'] = $this->photo->store('residents/photos');
        }
                        ...
                        <div class="mt-2 flex items-center gap-6">
                            <div class="shrink-0">
                                ...
                                @elseif($existing_photo)
                                    <img src="{{ Storage::url($existing_photo) }}" alt="" class="size-20 rounded-full object-cover" />
                                @else
                            ...
                            <div class="flex-1 space-y-2">
                                @if($existing_photo && !$photo)
                                    <flux:button variant="ghost" size="sm" wire:click="removePhoto" icon="trash" type="button">
                                        {{ __('Remove Photo') }}
                                    </flux:button>
                                @endif
                                <label class="flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-zinc-300 dark:border-zinc-600 p-4 hover:border-zinc-400 dark:hover:border-zinc-500 transition-colors">
                                    ...
                                    <span class="text-sm text-zinc-500">{{ __($existing_photo ? 'Click to change photo' : 'Click to upload') }}</span>
