...
class extends Component {
    ...
    {
        return ContactNote::where('resident_id', $this->residentId)
    ...
    {
        return ContactNote::where('resident_id', $this->residentId)->count();
    }
        ...
        <div class="grid grid-cols-3 gap-4">
            <flux:card class="flex items-center gap-3 p-4">
                <div class="flex size-10 items-center justify-center rounded-full bg-blue-50 dark:bg-blue-900/20">
                    <flux:icon name="phone-arrow-up-right" class="size-5 text-blue-600 dark:text-blue-400" />
                </div>
                <div>
            ...
            <flux:card class="overflow-hidden p-0">
                <flux:table>
                    <flux:table.columns>
                        ...
                        <flux:table.column>{{ __('Contact Date') }}</flux:table.column>
                        <flux:table.column>{{ __('Person Contacted') }}</flux:table.column>
                        <flux:table.column>{{ __('Emergency') }}</flux:table.column>
                    ...
                    <flux:table.rows>
                        @foreach ($this->records as $record)
                            <flux:table.row :key="$record->id">
                                ...
                                <flux:table.cell>{{ $record->contact_date->format('M d, Y') }}</flux:table.cell>
                                <flux:table.cell>
                                    @if (!empty($record->person_contacted))
                                        {{ implode(', ', $record->person_contacted) }}
                                    @else
                                ...
                                <flux:table.cell>
                                    <flux:button variant="ghost" size="sm" icon="arrow-top-right-on-square"
                                        :href="route('contact-notes.show', $record->id)" wire:navigate />
                                </flux:table.cell>
                            </flux:table.row>
                        @endforeach
                    </flux:table.rows>
