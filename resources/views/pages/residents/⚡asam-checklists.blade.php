...
#[Title('ASAM Criteria Checklists')]
class extends Component {
    ...
    {
        return AsamChecklist::where('resident_id', $this->residentId)
            ->with(['recorder', 'signature'])->latest()->paginate(15);
    }

    #[Computed]
    public function totalRecords(): int { return AsamChecklist::where('resident_id', $this->residentId)->count(); }
    ...
    <div class="max-w-5xl space-y-6">
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <flux:heading size="xl">{{ __('ASAM Criteria Checklists') }}</flux:heading>
                    <flux:subheading>{{ $this->resident->full_name }}</flux:subheading>
                </div>
            </div>
            @can('manage-residents')
                <flux:button variant="primary" icon="plus" :href="route('residents.asam-checklists.create', $this->residentId)" wire:navigate>
                    {{ __('New Checklist') }}
                </flux:button>
            @endcan
        </div>
        ...
        <div class="grid grid-cols-3 gap-4">
            <flux:card class="flex items-center gap-3 p-4">
                ...
                <div><div class="text-xl font-bold">{{ $this->totalRecords }}</div><div class="text-xs text-zinc-500">Total</div></div>
            </flux:card>
            <flux:card class="flex items-center gap-3 p-4">
        ...
        @if ($this->records->isEmpty())
            <flux:card class="flex flex-col items-center py-16 text-center">
                ...
                <flux:heading size="sm">{{ __('No ASAM checklists yet') }}</flux:heading>
                @can('manage-residents')
                    <flux:button class="mt-6" variant="primary" icon="plus" :href="route('residents.asam-checklists.create', $this->residentId)" wire:navigate>
                        {{ __('Create First Checklist') }}
                    </flux:button>
                @endcan
            </flux:card>
        @else
            <flux:card class="overflow-hidden p-0">
                <flux:table>
                    <flux:table.columns>
                        ...
                        <flux:table.column>Discharge Date</flux:table.column>
                        <flux:table.column>Residential</flux:table.column>
                    ...
                    <flux:table.rows>
                        @foreach ($this->records as $record)
                            <flux:table.row :key="$record->id">
                                ...
                                <flux:table.cell>{{ $record->discharge_date?->format('M d, Y') ?? '—' }}</flux:table.cell>
                                <flux:table.cell>
                                    @if ($record->residential)
                                        <flux:badge size="sm" color="blue">{{ $record->residential }}</flux:badge>
                                    @else
                                ...
                                <flux:table.cell>
                                    <flux:button variant="ghost" size="sm" icon="arrow-top-right-on-square"
                                        :href="route('asam-checklists.show', $record->id)" wire:navigate />
                                </flux:table.cell>
                            </flux:table.row>
                        @endforeach
                    </flux:table.rows>
