...
class extends Component {
    ...
    {
        return BhpProgressNote::where('resident_id', $this->residentId)
    ...
    {
        return BhpProgressNote::where('resident_id', $this->residentId)->count();
    }
    ...
    <div class="max-w-5xl space-y-6">
        <div class="flex flex-wrap items-start justify-between gap-4">
            ...
            @can('manage-residents')
                <flux:button variant="primary" icon="plus" :href="route('residents.bhp-progress-notes.create', $this->residentId)" wire:navigate>
                    {{ __('New BHP Note') }}
                </flux:button>
            @endcan
        </div>
        ...
        <div class="grid grid-cols-3 gap-4">
            <flux:card class="flex items-center gap-3 p-4">
                ...
                <div>
                    <div class="text-xl font-bold">{{ $this->totalRecords }}</div>
                    <div class="text-xs text-zinc-500">{{ __('Total Notes') }}</div>
                </div>
            </flux:card>
            <flux:card class="flex items-center gap-3 p-4">
                ...
                <div>
                    <div class="text-xl font-bold">{{ $this->recordsThisMonth }}</div>
                    <div class="text-xs text-zinc-500">{{ __('This Month') }}</div>
                </div>
            </flux:card>
            <flux:card class="flex items-center gap-3 p-4">
                ...
                <div>
                    <div class="text-xl font-bold">{{ $this->signedCount }}</div>
                    <div class="text-xs text-zinc-500">{{ __('Signed') }}</div>
                </div>
            </flux:card>
        </div>
            ...
            <flux:card class="overflow-hidden p-0">
                <flux:table>
                    <flux:table.columns>
                        ...
                        <flux:table.column>{{ __('Discharge Date') }}</flux:table.column>
                        <flux:table.column>{{ __('BHP Name') }}</flux:table.column>
                    ...
                    <flux:table.rows>
                        @foreach ($this->records as $record)
                            <flux:table.row :key="$record->id">
                                ...
                                <flux:table.cell>{{ $record->bhp_name_credential ?? '—' }}</flux:table.cell>
                                <flux:table.cell>
                                ...
                                <flux:table.cell>
                                    <flux:button variant="ghost" size="sm" icon="arrow-top-right-on-square"
                                        :href="route('bhp-progress-notes.show', $record->id)" wire:navigate />
                                </flux:table.cell>
                            </flux:table.row>
                        @endforeach
                    </flux:table.rows>
