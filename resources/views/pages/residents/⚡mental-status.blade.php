...
class extends Component {
    ...
    {
        return MentalStatusExam::where('resident_id', $this->residentId)
            ->with(['recorder', 'signature'])->latest()->paginate(15);
    }

    #[Computed]
    public function totalRecords(): int { return MentalStatusExam::where('resident_id', $this->residentId)->count(); }
    ...
    <div class="max-w-5xl space-y-6">
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <flux:heading size="xl">{{ __('Mental Status Examinations') }}</flux:heading>
                    <flux:subheading>{{ $this->resident->full_name }}</flux:subheading>
                </div>
            </div>
            @can('manage-residents')
                <flux:button variant="primary" icon="plus" :href="route('residents.mental-status.create', $this->residentId)" wire:navigate>
                    {{ __('New Exam') }}
                </flux:button>
            @endcan
        </div>
        ...
        <div class="grid grid-cols-3 gap-4">
            <flux:card class="flex items-center gap-3 p-4">
                <div class="flex size-10 items-center justify-center rounded-full bg-blue-50 dark:bg-blue-900/20">
                    <flux:icon name="face-smile" class="size-5 text-blue-600 dark:text-blue-400" />
                </div>
                <div><div class="text-xl font-bold">{{ $this->totalRecords }}</div><div class="text-xs text-zinc-500">Total Exams</div></div>
            </flux:card>
            <flux:card class="flex items-center gap-3 p-4">
        ...
        @if ($this->records->isEmpty())
            <flux:card class="flex flex-col items-center py-16 text-center">
                <div class="mb-4 flex size-16 items-center justify-center rounded-full bg-zinc-100 dark:bg-zinc-800">
                    <flux:icon name="face-smile" class="size-8 text-zinc-400" />
                </div>
                <flux:heading size="sm">{{ __('No mental status exams yet') }}</flux:heading>
                @can('manage-residents')
                    <flux:button class="mt-6" variant="primary" icon="plus" :href="route('residents.mental-status.create', $this->residentId)" wire:navigate>
            ...
            <flux:card class="overflow-hidden p-0">
                <flux:table>
                    <flux:table.columns>
                        ...
                        <flux:table.column>Exam Date</flux:table.column>
                    ...
                    <flux:table.rows>
                        @foreach ($this->records as $record)
                            <flux:table.row :key="$record->id">
                                ...
                                <flux:table.cell>{{ $record->exam_date->format('M d, Y') }}</flux:table.cell>
                                ...
                                <flux:table.cell>
                                    <flux:button variant="ghost" size="sm" icon="arrow-top-right-on-square"
                                        :href="route('mental-status.show', $record->id)" wire:navigate />
                                </flux:table.cell>
                            </flux:table.row>
                        @endforeach
                    </flux:table.rows>
