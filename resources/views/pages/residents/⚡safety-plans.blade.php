...
class extends Component {
    ...
    {
        return SafetyPlan::where('resident_id', $this->residentId)
            ->with(['recorder', 'signature'])->latest()->paginate(15);
    }

    #[Computed]
    public function totalRecords(): int { return SafetyPlan::where('resident_id', $this->residentId)->count(); }
        ...
        <div class="grid grid-cols-3 gap-4">
            <flux:card class="flex items-center gap-3 p-4">
                <div class="flex size-10 items-center justify-center rounded-full bg-blue-50 dark:bg-blue-900/20">
                    <flux:icon name="shield-check" class="size-5 text-blue-600 dark:text-blue-400" />
                </div>
                <div><div class="text-xl font-bold">{{ $this->totalRecords }}</div><div class="text-xs text-zinc-500">Total Plans</div></div>
            </flux:card>
            <flux:card class="flex items-center gap-3 p-4">
                            ...
                            <flux:table.row :key="$record->id">
                                ...
                                <flux:table.cell>
                                    <flux:button variant="ghost" size="sm" icon="arrow-top-right-on-square"
                                        :href="route('safety-plans.show', $record->id)" wire:navigate />
                                </flux:table.cell>
                            </flux:table.row>
                        @endforeach
                    </flux:table.rows>
